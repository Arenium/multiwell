...
c
      ...
      REAL(8) :: EJmin                            ! K-rotor zero of energy = VZPE - EJmin
      ...
      REAL(8), DIMENSION(NMAX) :: SUM
      REAL(8), DIMENSION(NMAX) :: DENS
...
!
      ...
      write(lo,*) '  '
      write(lo,*) '      ',
     & 'Index           J           K         cm-1                degen'
 
      jj = 0
      R = 0.0d+00
      DO J = 1 , jjmax                                    ! start at qantum number J=1
          Ej = B2*J*(J + 1.d+00)
          DO k = 0 , J                                    ! K quantum number
             ...
             IF ( R.GE.0.0 .AND. R.LE.EMAX) THEN
                ...
                ENDIF
                IF ( R .LE. 5000. ) write(lo,*) jj , J , k , R , F       ! write out the energy level and degeneracy for R ≥ 5000 cm-1                ENDIF
...
!-----------------------------------------------------------------------
      ...
      Subroutine chkdens(Egrain1,Imax1,Isize,Emax2,bestImax)
!
...
!-----------------------------------------------------------------------
      SUBROUTINE KROTLEV( T, DELE, JMAX, B2, B1, J, Emin )
!
...
!
      ...
      REAL(8), INTENT(OUT) :: Emin                     ! lowest J,K rotational energy for given J
      ...
      IF ( B1 .GE. B2 ) THEN                              ! Prolate & spherical
         Emin = B2*J*(J + 1.0d+00)                           ! energy of lowest J,K level when J>0
      ELSEIF ( B1 .LT. B2 ) THEN                          ! Oblate
         Emin = B2*J*(J + 1.d+00) + ( B1 - B2 )*J*J          ! energy of lowest J,K level when J>0
      ENDIF
      Imin = NINT(Emin/DELE) + 1
      
      IF ( Imin .GT. JMAX ) THEN              ! RETURN IF LOWEST (J,K) STATE IS HIGHER THAN Emax
        RETURN
      END IF

      write(lo,9901) Emin
      write(lo,9902)
9901  FORMAT(/, ' K-rotor: Emin /cm-1 = ', F9.2, /)
9902  FORMAT( 7X, 'Index',11X, 'J',11X, 'K',3X, '(E - Emin)/cm-1',
     &        15x, 'degen', 10x, 'IR' )

      Fj = 2.0d+00*J + 1.0d+00
      jj = 0
      R = 0.0d+00
      DO K = 0 , J              ! start with K=0 instead of k=1 because k=0 is not necessarily lowest energy K-state
                                ! starting with K=0 requires subsequent correction for double-counting
         jj = jj + 1                                    ! = level number
         R = B2*J*( J + 1.d+00 ) + ( B1 - B2 )*K*K  - Emin
         IR = NINT( R/DELE ) + 1                        ! Nearest integer: number of grains
         IF ( K .EQ. 0 ) THEN
            F = Fj
          ELSE
            F = 2.0d+00*Fj
         ENDIF
         IF ( jj.LT. 1001.) write(lo,*) jj , J , K , R , NINT(F), IR  ! write out the energy level and degeneracy for ≤ 1000 states
         DO kk = IR , JMAX                            ! Jmax = number of energy grains (i.e. corresponding to Emax2)
            KARG = kk - IR + 1
            IF ( KARG.GT.0 .AND. KARG.LE.JMAX) THEN
               AT(kk) = AT(kk) + F*T(KARG)
            ENDIF
      ...
      AT(1) = AT(1) - 1.0d+00    ! correction for over-counting of lowest state
      
      DO I = 1 , JMAX     ! over energy grains
         T(I) = AT(I) + T(I)                             ! includes 2J+1 degeneracy
      END DO  !  I
